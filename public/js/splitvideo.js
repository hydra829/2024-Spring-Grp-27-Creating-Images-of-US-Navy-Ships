...
function onReady () {
    ...
    function download(v_count) {
        var cop = document.getElementById('canvasOutput'.concat(v_count));
        let link = document.createElement('a');
        link.download = 'image'.concat(v_count);
        
        try {
            link.href = cop.toDataURL('image/jpg');
        } 
        catch (error) {
            alert(error);
        }
